...
function renderEditor(reviseChat: (unitId: string, instruction: string) => Promise<RevisionSuggestionPayload | undefined>) {
  useProjectUnits.mockReturnValue({
    units: [preparedUnit], loading: false, error: null, reload: vi.fn(), add: vi.fn(), update: updateUnit,
    generate: vi.fn(), split: vi.fn(), mergeNext: vi.fn(), reviseChat,
    evaluate: vi.fn(), evaluateIntegrated: vi.fn(), verify: vi.fn(),
  });
...
describe("EditorPage collaborative revision compatibility", () => {
  ...
  it("sends edited collaborative content to backend accept and never directly patches the DraftUnit", async () => {
    const reviseChat = vi.fn().mockResolvedValue({ kind: "collaborative", work: collaborativeWork } satisfies RevisionSuggestionPayload);
    const integratedUnit = { ...preparedUnit, content: "Proposition éditée.", version: 4 };
    ...
    await vi.waitFor(() => expect(acceptWork).toHaveBeenCalledWith("project-1", "unit-prepared", collaborativeWork.id, "Proposition éditée."));
    expect(updateUnit).not.toHaveBeenCalled();
    expect(await screen.findByRole("textbox", { name: "Manuscrit : Unité préparée" })).toHaveValue("Proposition éditée.");
  ...
  it("rejects collaborative work through the backend without changing canonical text", async () => {
    const reviseChat = vi.fn().mockResolvedValue({ kind: "collaborative", work: collaborativeWork } satisfies RevisionSuggestionPayload);
    rejectWork.mockResolvedValue({ kind: "collaborative", status: "rejected", work: { ...collaborativeWork, status: "rejected" } });
    renderEditor(reviseChat);
    ...
    expect(updateUnit).not.toHaveBeenCalled();
    expect(screen.getByRole("textbox", { name: "Manuscrit : Unité préparée" })).toHaveValue("Texte canonique.");
  });

  it("keeps collaborative apply backend-authoritative when local text makes the proposal look stale", async () => {
    const reviseChat = vi.fn().mockResolvedValue({ kind: "collaborative", work: collaborativeWork } satisfies RevisionSuggestionPayload);
    acceptWork.mockResolvedValue({
      kind: "collaborative", status: "stale", work: { ...collaborativeWork, status: "stale" },
      conflicts: [{ kind: "textual", reason: "Le texte canonique a évolué." }],
    });
    renderEditor(reviseChat);

    await requestRevision();
    fireEvent.change(screen.getByRole("textbox", { name: "Manuscrit : Unité préparée" }), { target: { value: "Modification locale concurrente." } });
  ...
  it("keeps legacy proposals on the existing explicit local apply path", async () => {
    const legacyProposal = {
      id: "proposal-legacy", projectId: "project-1", unitId: preparedUnit.id, sourceVersion: 3,
      before: preparedUnit.content, content: "Proposition legacy.", status: "available" as const,
    ...
    await vi.waitFor(() => expect(updateUnit).toHaveBeenCalledWith("unit-prepared", { content: "Proposition legacy.", version: 4 }));
